...
void bmp_write(const BMP *bmp, Buffer *buf) {
    ...
    for (uint32_t y = 0; y <= bmp->height - 1; y++) {
        ...
        for (int32_t z = bmp->width; z <= rounded_width - 1; z++) {
            write_uint8(buf, 0);
        }
    }
}
...
BMP *bmp_create(Buffer *buf) {
    ...
    if (bmp == NULL) {
        //look back at this because i dont know if this is the right return
        return 0;
    }
    //could just use one variable for all skips
    uint8_t type1;
    uint8_t type2;
    uint32_t skip_32;
    uint8_t skip_8;
    ...
    bool check;
    check = read_uint8(buf, &type1);
    if (type1 != 'B' || check == false) {
        return NULL;
    }
    check = read_uint8(buf, &type2);
    if (type2 != 'M' || check == false) {
        return NULL;
    }
    check = read_uint32(buf, &skip_32);
    if (check == false) {
        bmp_free(&bmp);
        return NULL;
    }
    check = read_uint16(buf, &skip_16);
    if (check == false) {
        bmp_free(&bmp);
        return NULL;
    }
    ...
    check = read_uint32(buf, &bitmap_header_size);
    if (bitmap_header_size != 40 || check == false) {
        bmp_free(&bmp);
        return NULL;
    }

    check = read_uint32(buf, &bmp->width);
    if (check == false) {
        bmp_free(&bmp);
        return NULL;
    }

    check = read_uint32(buf, &bmp->height);
    if (check == false) {
        bmp_free(&bmp);
        return NULL;
    }
    ...
    check = read_uint16(buf, &bits_per_pixel);
    if (bits_per_pixel != 8 || check == false) {
        bmp_free(&bmp);
        return NULL;
    }

    check = read_uint32(buf, &compression);
    if (compression != 0 || check == false) {
        bmp_free(&bmp);
        return NULL;
    }
    ...
    check = read_uint32(buf, &colors_used);
    if (check == false) {
        bmp_free(&bmp);
        return NULL;
    }
    ...
    for (uint32_t i = 0; i <= num_colors - 1; i++) {
        check = read_uint8(buf, &bmp->palette[i].blue);
        if (check == false) {
            bmp_free(&bmp);
            return NULL;
        }

        check = read_uint8(buf, &bmp->palette[i].green);
        if (check == false) {
            bmp_free(&bmp);
            return NULL;
        }
        check = read_uint8(buf, &bmp->palette[i].red);
        if (check == false) {
            bmp_free(&bmp);
            return NULL;
        }
        check = read_uint8(buf, &skip_8);
        if (check == false) {
            bmp_free(&bmp);
            return NULL;
        }
    ...
    for (uint32_t y = 0; y <= bmp->height - 1; y++) {
        for (uint32_t z = 0; z <= rounded_width - 1; z++) {
            read_uint8(buf, &bmp->a[z][y]);
        }
    }
    return bmp;
}
void bmp_free(BMP **bmp) {
    uint32_t rounded_width = ((*bmp)->width + 3) & ~3;
    //remember to look back at this if i have errors so just switch to ++i)
